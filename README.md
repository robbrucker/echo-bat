...
Echo Bat ships as a real native iOS app via [Capacitor](https://capacitorjs.com/) — the same TypeScript/Canvas game runs inside a `WKWebView`, with native CoreMotion replacing the flaky Safari `DeviceOrientation.requestPermission` flow.

```bash
npm run ios:sync   # build web + copy into ios/
npm run ios:open   # build, sync, open in Xcode
npm run ios:run    # build, sync, run on connected device / simulator
```

First-time setup in Xcode:

1. `npm run ios:open`.
2. In the project navigator, select the **App** target → **Signing & Capabilities**.
3. Pick your **Team** (free Apple ID works for sideload to your own device; paid Developer account needed for App Store).
4. Pick a simulator (iPhone 16 etc.) or a connected device, hit **Run**.
